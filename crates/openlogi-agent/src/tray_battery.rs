...
//! looking at.
//!
//! Rows are localized when the snapshot is built, from the shared catalogs
//! the rest of the tray reads. A language switch reaches them on the next
//! inventory change rather than instantly; the menu's own items, built on
//! every right-click, follow it at once.
...
pub fn label(device: &TrayDevice) -> String {
    ...
    if charging_no_reading {
        rust_i18n::t!("app.tray_battery_charging", name = device.name.as_str()).into_owned()
    } else {
        rust_i18n::t!(
            "app.tray_battery_percentage",
