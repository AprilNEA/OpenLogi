...
use crate::features::profiles::{AppCatalogPicker, ApplicationIconState, ProfileIconCache};
    ...
    let application_icon =
        entry
            ...
            .and_then(|target| match app_icons.state(target) {
                ApplicationIconState::Ready(icon) => Some(icon),
                ApplicationIconState::Loading | ApplicationIconState::Missing => None,
            });
