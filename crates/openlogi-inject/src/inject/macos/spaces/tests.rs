...
/// concurrent trackpad/keyboard Space switching. Leaves that display one Space
/// to the right; never changes keyboard settings, moves the cursor, or retries.
#[test]
#[ignore = "changes the current macOS Space; requires explicit interactive execution"]
fn interactive_next_space() {
    autoreleasepool(|_| {
        ...
        let result =
            space_switch::run(&mut backend, Direction::Next).expect("Space switch confirmed");
