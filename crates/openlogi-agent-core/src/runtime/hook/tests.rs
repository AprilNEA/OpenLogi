...
#[test]
fn os_hook_response_time_does_not_control_gesture_admission() {
    let source_id = EventDeviceId::new("mouse-b").unwrap();
    let source = EventDevice {
        stable_id: Some(source_id.clone()),
        ..EventDevice::default()
    };
    let selected_gesture = HookMaps {
        gestures: BTreeMap::from([(ButtonId::Back, BTreeMap::new())]),
        gesture_response_times: BTreeMap::from([(ButtonId::Back, GestureResponseTime::FAST)]),
        gesture_response_times_by_source: BTreeMap::from([(source_id.clone(), BTreeMap::new())]),
        ..HookMaps::default()
    };
    assert_eq!(
        selected_gesture.gesture_response_time_for(Some(&source), ButtonId::Back),
        Some(GestureResponseTime::FAST)
    );

    let source_only_gesture = HookMaps {
        gesture_response_times_by_source: BTreeMap::from([(
            source_id,
            BTreeMap::from([(ButtonId::Back, GestureResponseTime::DELIBERATE)]),
        )]),
    ...
    assert_eq!(
        source_only_gesture.gesture_response_time_for(Some(&source), ButtonId::Back),
        None
    );
}

#[test]
fn os_hook_response_time_uses_selected_then_balanced_fallbacks() {
    let unknown = EventDevice {
        stable_id: Some(EventDeviceId::new("unknown-mouse").unwrap()),
        ..EventDevice::default()
    };
    let selected = HookMaps {
        gestures: BTreeMap::from([(ButtonId::Back, BTreeMap::new())]),
        gesture_response_times: BTreeMap::from([(ButtonId::Back, GestureResponseTime::FAST)]),
        ..HookMaps::default()
    };
    assert_eq!(
        selected.gesture_response_time_for(None, ButtonId::Back),
        Some(GestureResponseTime::FAST)
    );
    assert_eq!(
        selected.gesture_response_time_for(Some(&unknown), ButtonId::Back),
        Some(GestureResponseTime::FAST)
    );

    let balanced = HookMaps {
        gestures: BTreeMap::from([(ButtonId::Back, BTreeMap::new())]),
        ..HookMaps::default()
    };
    assert_eq!(
        balanced.gesture_response_time_for(None, ButtonId::Back),
        Some(GestureResponseTime::BALANCED)
    );
}

#[test]
fn os_hook_response_identity_normalizes_native_unit_formats() {
    assert_eq!(
        EventDeviceId::new("7C-85-DC-B1"),
        EventDeviceId::new("7c85dcb1")
    );
    assert_ne!(
        EventDeviceId::new("7C-85-DC-B1"),
        EventDeviceId::new("7C-85-DC-B2")
    );
}
