...
/// them (gpui-component#2701 dropped Matrix), and a hard-coded list turns either
/// into a broken build — or a theme silently missing from the picker — the next
/// time the pin moves. Our own `themes/openlogi.json` lives in this repo, not in
/// that directory, so nothing has to be filtered out here.
