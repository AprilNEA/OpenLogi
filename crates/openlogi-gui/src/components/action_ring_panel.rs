...
use openlogi_core::binding::{ActionRingIcon, ActionRingLayout, ActionRingSlot, RingAction};
            ...
            .child(
                h_flex()
                    ...
                    .child(action_library(
                        ...
                        ring.default.icons.get(&self.selected_slot).copied(),
                        &application_input,
                        &shortcut_input,
                        pal,
                    )),
...
fn slot_button(
    ...
    custom_icon: Option<ActionRingIcon>,
    selected: bool,
    view: &Entity<ActionRingPanel>,
    pal: Palette,
) -> impl IntoElement {
    let index = ActionRingSlot::ALL
        .iter()
    ...
    let label = action.map_or_else(
        ...
        |action| rust_i18n::t!(action.action().label()).into_owned(),
    );
    let icon_path = action.map(|action| {
        custom_icon.map_or_else(|| action_icon_path(action.action()), ring_icon_path)
    });
