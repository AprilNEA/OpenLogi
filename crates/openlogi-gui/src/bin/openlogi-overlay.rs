...
impl RingView {
    ...
    fn slot_element(
        ...
        let action = self.invocation.slots.get(&slot)?;
        let icon_path = self.invocation.icons.get(&slot).map_or_else(
            || action_icons::action_icon_path(action),
            |icon| action_icons::ring_icon_path(*icon),
        );
...
impl Render for RingView {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let hovered_label = self.hovered.and_then(|slot| {
            ...
            Some(SharedString::from(rust_i18n::t!(key.as_str()).into_owned()))
        });
...
fn init_locale() {
    let system = sys_locale::get_locale().unwrap_or_else(|| "en".to_string());
    let normalized = system.replace('_', "-");
    let lowercase = normalized.to_ascii_lowercase();
    let locale = match lowercase.as_str() {
        value if value.starts_with("pt-br") => "pt-BR",
        value if value.starts_with("pt") => "pt-PT",
        value if value.starts_with("zh-hk") => "zh-HK",
        value if value.starts_with("zh-tw") => "zh-TW",
        value if value.starts_with("zh") => "zh-CN",
        value if value.starts_with("nb") || value.starts_with("no") => "nb",
        value => value.split('-').next().unwrap_or("en"),
    };
