...
Pre-built `.deb`, `.rpm`, `.pkg.tar.zst`, and portable `.tar.gz` packages are available on the
...
`openlogi-<version>-linux-<arch>.tar.gz`, and then uses `nfpm` to create
`.deb`, `.rpm`, and `.pkg.tar.zst` packages in `target/release/`.

For local testing without rebuilding binaries:

```sh
cargo run -p xtask -- linux package --no-build
```
